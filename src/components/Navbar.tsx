  ...
  return (
    <nav className="bg-gray-800 text-white p-1 fixed top-0 w-full z-20">
      <div className="container mx-auto flex justify-between items-center">
        ...
        {/* Navbar Links */}
        <div className={`flex-col md:flex md:flex-row ${isOpen ? 'flex' : 'hidden'} md:block transition-all duration-200 ease-in-out`}>
          {/* Registration Dropdown */}
          <div className="relative group">
            <button className="px-8 py-6 hover:text-gray-400 focus:outline-none">Registration</button>
            <div className="absolute left-0 mt-1 w-56 bg-white text-black shadow-lg rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 ease-in-out pointer-events-none group-hover:pointer-events-auto z-50">
          ...
          <div className="flex items-center space-x-4">
            {!session ? (
              <button onClick={() => signIn('google')} className="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded">
                Sign In
              </button>
            ) : (
              <>
